...
def generate_response(history, query, btn):
    ...
    if COUNT == 0:
        print("Creating new index for first time")
        chain = process_file(btn)
        COUNT += 1
    else :
        print("Deleting old index and recreating new one")
        if os.path.exists('.chroma'):
            #vectorstore = pdfsearch.vectorstore            
            pdfsearch.delete_collection()
            directory = ".chroma"
            print("Deleting chroma db")
